...
public class Tableaux {
    ...
    */
    ...
    public static boolean test(){
        int [] x = new int[1000];
    // tirer 1000 nombres au hasard pour remplir x (utiliser Random)
        //boucle ... de 0 à 999 ...
        Random r = new Random();
        for(int i=0;i<x.length;i++){
            x[i]=r.nextInt();
    ...
    // vérifier que le tableau est trié
        boolean v = true;
        // boucle ... de 0 à 998 ...
            //if deux trucs pas dans l'ordre v<-false
        for(int i=0;i<x.length-1;i++){
            if (x[i]<x[i+1]) {v=false;}
        }
    // retourner true si il est trié, false sinon
        return v;
    }
